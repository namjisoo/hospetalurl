...
const LikeDislikeChartPh = ({
  ...
  useEffect(() => {
    const storedLikeCount = parseInt(
      localStorage.getItem(`${pharmacyTitle}-likeCount`),
      10
    );
    const storedDislikeCount = parseInt(
      localStorage.getItem(`${pharmacyTitle}-dislikeCount`),
      10
    );
    ...
    if (!isNaN(storedLikeCount)) {
      ...
      localStorage.setItem(`${pharmacyTitle}-likeCount`, initialLikeCount || 0);
    }
    ...
    if (!isNaN(storedDislikeCount)) {
      ...
      localStorage.setItem(
        `${pharmacyTitle}-dislikeCount`,
        initialDislikeCount || 0
      );
    }
  }, [pharmacyTitle, initialLikeCount, initialDislikeCount]);

  const handleLikeClick = () => {
    const updatedLikeCount = likeCount + 1;
    setLikeCount(updatedLikeCount);
    localStorage.setItem(`${pharmacyTitle}-likeCount`, updatedLikeCount);
  };

  const handleDislikeClick = () => {
    const updatedDislikeCount = dislikeCount + 1;
    setDislikeCount(updatedDislikeCount);
    localStorage.setItem(`${pharmacyTitle}-dislikeCount`, updatedDislikeCount);
  };
...
export default LikeDislikeChartPh;
